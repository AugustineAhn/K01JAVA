...
/*
 ...
 -반환값이 없다. 즉 특정 연산 후 값을 반환할 수 없다.
 		따라서 void를 쓸 수 없다.
 -그외는 메소드가 가진 모든 특성을 가지고 있다.(매개변수전달, 오버로딩 등)
 ...
 용도1 : 동일 클래스내에서 다른 생성자를 호출할때 사용한다.
 	...
 	사용법:
 		this(인자1, 인자2,....);
 용도2: 생성자 내에서 멤버변수와 매개변수의 이름이 동일할 때
 		구분하기 위해 사용한다.
 		this.변수 => 멤버변수
 		변수=> 생성자의 매개변수
...
public class PersonConstructor {
	...
	void initialize(String name, int age, String addr) {
		...
		//this(name, age,"미상");또다른 생성자를 호출하는 this
