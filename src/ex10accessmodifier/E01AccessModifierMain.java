...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 ※private으로 선언된 멤버를 외부 클래스에서 접근할 때는 주로
		 public 으로 선언된 멤버메소드를 통해 간접적으로 접근한다.
		 */
		E01AccessModifier1 one = new E01AccessModifier1();
		
		//System.out.println("one.privateVar=" + 
			//one.privateVar);//에러발생
		...
		/*
		 다른패키지에 선언된 클래스 이므로 객체생성을 위해서는 import가 
		 필요하다.
		...
		//two.defaulMethod();//에러발생
		two.publicMethod();
		
		/*
		 다른 패키지에 정의된 디폴트클래스이므로 객체생성이 불가능하다.(같은 패키지에서만)
