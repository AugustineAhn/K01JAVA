...
/*
 ...
 -RuntimeException 클래스 계열로 NullPointException이 
 대표적인 하위 클래스이다.
 -main 메소드에서 무시하기 위해 throws하더라도 오류가 발생되므로
 반드시 예외처리를 해줘야한다.
 -예외 발생시 JVM은 해당 예외 클래스를 인스턴스화해서 프로그램쪽으로
 전달하게 되고try~catch 애서 받은 후 처리한다.
 */
public class Ex02PreDefineException {

	//참조변수를 선언만하고 객체생성이 되지 않은 경우
	static Date toDay;
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 :배열의 크기를 벗어난 인텍스를 사용할 때 발생되는 예외
		 */
		...
		try {
			...
			/*
			 ...
			 printStackTrace: 예외가 발생할 때의 에러베시지를
		...
		/*
		 ...
		 : 숫자형식의 문자열을 int로 변환할 경우 해당 문자열이
		 숫자형식이 아니면 발생되는 예외
		 */
		...
		try {
			...
			System.out.println("나이는 문자형태로 입력하면 앙데요.");
			System.out.println("예외메세지:" + e.getMessage());
			e.printStackTrace();
		}
		 ...
		 */
		Object object = new Object();//Object object => Stack, new Object()=> Heap.
