...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/*
		 3] 객체저장
		 	: add() 메소드를 통해 객체가 정상적으로 저장됨녀
		 	true를 반환한다.
		 */
		System.out.println(set.add(strObject1));//true반환
		set.add(strObject2);
		set.add(dateObject);
		set.add(number);//<- new Integer(number) wrapping처리됨
		set.add(teacher);
		...
		System.out.println("[중복(String)저장후 객체수]:"+ set.size());//15개

		/*
		 5-2] 개발자가 정의한 객체 중복저장
		 	: 직접 정의한 클래스인 경우 hashCode, equals메소드를 오버라이딩 하지 않으면
		 	중복저장이 허용된다. 아래의 경우 저장에 성공하여 true가 반환된다.
		 */
		...
		System.out.println("[전체삭제 후 객체수]"+ set.size());
