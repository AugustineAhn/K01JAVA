...
class MyString implements Comparable<MyString>{
	...
	/*
	 정렬의 기준을 위해 오버라이딩한 메소드로
	 가나다와 같은 사전순, 문자열의 길이 등 다양하게 설정 가능하다.
	 아래 코드는 오름차순(작은것-> 큰것) 정렬일때 사용한다.
	 1과 -1의 위치가 바귀면 내림차순이 된다.
	 */
	@Override
	public int compareTo(MyString paramStr) {
		//정렬의 순서는 물자열의 길이를 기준으로 한다.
...
public class Ex04TreeSetMain {
	...
	public static void main(String[] args) {
		...
		tree1.add(2);
		tree1.add(3);
		System.out.println(tree1.add(2));//Set은 중복저장이 허용되지 않음
