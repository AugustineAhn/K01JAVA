...
public class Ex05HashMapMain {
	...
	public static void main(String[] args) {
		...
		//Map 컬렉션 생성, key와 value는 String형으로 선언
		...
		int number = 20;
		//map.put("age", number);//[에러발생] value 타입이 맞지 않음
		...
		/*
		 4. 중복저장
		 	: 기존에 입력된 name 이라는 key값이 있으므로
		 	"홍길동"이 출력된다. 그리고 기존의 값이 "최길동"으로 
		 	갱신된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값:"+ 
				map.put("name", "최길동"));//덮어쓰기 처리
		System.out.println("키값으로 중복 저장 후 객체수:"+ map.size());//4개
		...
		/*
		 5-2] 키값을 모를때 출력하기
		 	...
		 	Set 계열의 컬렉션 형태로 가져온다.
		 	2. Map 계열의 컬렉션에는 처음부터 확장 for문을 사용하는
		 	것이 불가능하다.
		 	3. 키값을 얻어온 후 확장 for문을 통해 key값을 하나씩
		 	출력한다.
		 */
		Set<String> keys = map.keySet();//배열 형태로 Set한다.
		...
		/*
		 6]존재 유무 판단
		 	: 존재할 경우 true를 반환한다.
		 */
		System.out.println(map.containsKey("name")? "name키값 있다": "name키값없다");
		System.out.println(map.containsKey("account")? "accounmt키값 있다": "account키값없다");
