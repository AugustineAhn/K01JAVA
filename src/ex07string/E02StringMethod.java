...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*
		 5] concat() : 두개의 문자열을 연결할 때 사용한다. +기호와
		 		동일한 역할을 한다.
		 */
		...
		/*
		 6] contains() : 문자열에 특정 문자열이 포함되어 있는지
		...
		/*
		 7] endsWith()
		 	...
		 	false를 반환한다.
		 8] startsWith()
		 	: 문자열이 특정문자열로 시작하면 true, 아니면
		 	false를 반환한다.
		 */
		System.out.println("www.daum.net".endsWith("net"));
		System.out.println("www.naver.com".endsWith("http"));
		...
		System.out.println((email1.indexOf("@")!=-1) ?//삼항연산자
				"이메일 형식맞음 " : "이메일 형식 아님");//조건  ?  참 : 거짓
		...
		int index = juminNum.indexOf("-")+1;//3의 위치(7번인덱스)를 명시하기위해 +1
		...
		for (int i = 0; i < phoneArr.length; i++) {
			System.out.printf("phonArr[%d]=%s\n", 
					i, phoneArr[i]);
		}
		
		/*
		 14] substring()
		 	: 시작 인덱스와 끝인덱스 사이의 문자열을 잘라서 반환해줌.
