...
class YesThread extends Thread{
	...
	public YesThread() {}
	/*
	 Thread 클래스의 생성자를 호출할때 전달하는 매개변수를
	 통해 해당 쓰레드의 이름을 지정할 수 있다. 지정된 이름은 
	 getName()메소드를 통해 반환할 수 있다.
	 */
...
/*
 종속쓰레드 (Daemon Thread)
 	...
 	말한다. 주로 프로그램의 보조역할로 배경음악, 로그 자동저장
 	등의 업무를 처리한다.
 	프로그램이 종료될 때 자동으로 종료되므로 주로 무한 루프로 
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		yt1.start();//쓰레드 객체를 Runnable 상태로 만들어준다.
		...
		System.out.println("현재 실행중인 상태의 쓰레드수 "+ Thread.currentThread().getName());
