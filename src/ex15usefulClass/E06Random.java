...
public class E06Random {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 만약 seed 값이 동일하다면 항상 동일한 패턴의 난수만 생성되는데
		...
		for (int i = 0; i < 10; i++) {
			/*
			 nextInt()의 인자값 100이 전달되면 0~99 사이의
			 난수가 생성된다.
			 */
		...
		System.out.println("### 난수생성4 : 다른 seed를 사용, nextInt() 인자없음");
		Random random4 = new Random();
		//현재 시간을 밀리 초 단위로 반환하는 메소드
		random4.setSeed(System.currentTimeMillis());
		for (int i = 0; i < 10; i++) {
			/*
			 nextInt()의 인자값이 없는 경우에는 음수, 양수가 혼용되어
			 난수가 생성된다.
			 */
