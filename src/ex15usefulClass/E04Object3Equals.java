...
class MyClass {
	...
	/*
	 MyClass 형의 객체의비교를 위해 오버라이딩한 메소드
	 객체가 가지고 있는 멤버변수의 비교를 통해 동일여부를 판단한다.
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		 매개변수로 전달된 객체를 Object로 받은 후 MyClass의 인스턴스 인지 
		 먼저 판단한다.
		 */
		if (obj instanceof MyClass) {
			//MyClasss형으로 다운캐스팅 진행
			...
			if (mc.data == this.data) {
				...
				비교의 대상이 도리수 없으므로 false반환함
				 */
				return false;//다를 때 false
			}
		}
		else {
			return false;
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
			...
			System.out.println("두 점의 비교 결과:");
			System.out.println(pos1.equals(pos2)?"같다 ": "다르다");

			/*
			 아래 toString() 메소드는 굳이 호출하지 않아도 print()문이 
			 실행될 때 자동으로 호출하므로, 아래 출력결과는 동일하다.
			 */
			System.out.println("pos1.toSting()=>"+pos1.toString());
			System.out.println("pos2.toSting()=>"+pos2.toString());
