...
/*
 ...
 메소드를 정의하시오. 단, 숫자의 갓수는 매개변수로 전달받고
 숫자의 갓수만큼 Scanner클래스를 통해 입력받는다.
 입력받은 숫자 중 최대값을 찾아서 반환하도록 정의 한다.
	...
	static int returnMaxNumber(int numberCnt) {
		...
		for (int i = 1; i < numberCnt; i++) {
			...
			if (i==1) {
				...
				//두번째 입값부터 maxVal과 비교한다.
				if(maxVal<inputNum) {
					/*
					 만약 maxVal 보다 입력값이 더 크다면 입력값으로 교체한다.
					 */
					maxVal = inputNum;
					//maxVal이 더 크다면 기존값을 유지하면 되므로 else는 피룡업삳.
				}
			}
