...
/*
 컴퓨터를 표현
 	:기본적인 컴퓨팅 호나경을 제공하여 입력된 내용을
 	계산한다.
 */
class Computer{
	String owner;//컴퓨터의 소유주이름
	...
	/*
	 private 접근지정자로 선언된 멤버메소드는 외부 클래스
	 혹은 상속받은 클래스에서 접근이 불가능하므로
	 public으로 선언된 메소드를 통해서 간접호출을 해야 한다.
	 상속관계라면 protected로 선언된 메소드로 호출할 수 있다.
	 */
	private void keyboardTouch() {//protected로 바꾸어도 무방
...
/*
 컴퓨터를 상속한 노트북컴퓨터
 	:컴퓨터가 가지고 있는 기본지능에 휴대이용 부분을 확장하여
...
class NotebookComputer extends Computer{
	...
	/*
	 ...
	 앞에 다른 문장이 먼저 오게되면 오류가 발생되므로 주의해야 한다.
	 */
	public NotebookComputer(String name, int initCharge) {
		super(name);//하위생성자 생성시 부모를 먼저 쓴다. 반드시. 만약 생략을 하게 되면 
		//디폴트슈퍼가 자동생성되고 디폴트생성자도 자동으로 만들어지는데 부모에는 이미 public이 들어 있으므로 생략할 수 없다.
		battary = initCharge;
	}
	//배터리 충전을 표현
...
class TabletNotebook extends NotebookComputer{
	...
	//태블릿 사용의 위해 배터리양 체크 및 등록된 펜인지 체크
