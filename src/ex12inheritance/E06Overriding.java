...
/*
 오버라이딩(Overriding)
 	: 클래스가 상속관계에 있을 때 하위 클래스에서 상위 클래스의
 	멤버 메소드와 완전히 동일한 모양으로 재정의 하는 것을 
 	말한다.
 	오버로딩(OverLoading)과 다른 점은 매개변수의 갯수, 타입,
...
class BaseSpeaker extends Speaker{
	...
	public void showState() {
		...
		System.out.println("베이스의볼륨크기:"+baseRate);
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		 ...
		 이 경우 접근 범위는 부모클래스로 제한된다.
		 즉 아래 호출부분에서 speaker 참조변수를 통해서는 setBase()
		 메소드는 호출할 수 없다. setBase()는 자식 클랙스에 정의된
		 멤버메소드이기 때문이다.
		 단, 오버라이딩 처리된 메소드가 있는 경우 하위클래스의 메소드가
		 호출되게 된다.
		 */
