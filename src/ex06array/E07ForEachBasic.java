...
/*
 for each문
 	: 배열의 일부가 아니라 전체를 참조해야 할 때 주로 사용하는
 	개선된 for문이다.
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		 배열arr의 요소를 하나씩 참조하여 출력하고, 더 이상
		 참조할 요소가 없을 때 for문이 자동으로 종료된다.
		 */
		sum = 0;
		
		for (int e : arr) {//인덱스가 없다. 더이상 출력할 것이 없으면 끝
			System.out.print(e +" ");
				sum += e;
		}
		System.out.println("\n foredach문을 이용한 합계:" +sum);
		
		
		
		/*
		 참조형태로 각 요소를 가져와서 변수b에 할당하는 형식이므로
		 아래 증가연산자를 통해 증가하는 것은 실제 배열에 적용되지
		 않는다. 해당 for each문 내에서만 증가하게 된다.
		 */
		...
		for (int c : arr) {
		System.out.print(c + " ");//*+1증가된 부분이 기존배열에 적용되지 않으므로
//										기존의 값이 그대로 출력된다.*//
		}
		System.out.println();
